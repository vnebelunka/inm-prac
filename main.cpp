 ...
 * @param A original matrix.
 */
void Tridiagonaliation(Matrix<double> const &A, Matrix<double>  &D, Matrix<double>  &Q) {
    size_t n = A.size().first;
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            D(i, j) = A(i, j);
 ...
 * @param Q
 */
void check(Matrix<double> const &A, Matrix<double> const &D, Matrix<double> const &Q){
    size_t n = A.size().first;
...
int main() {
    ...
    Matrix<double> A(n, n), D(n, n), Q(n, n);
    for(int i = 0; i < n; ++i){
        for(int j = i; j < n; ++j){
            A(i, j) = distr(eng);
            A(j, i) = A(i, j);
